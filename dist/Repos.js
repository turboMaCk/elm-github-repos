...
Elm.Repos = Elm.Repos || {};
Elm.Repos.make = function (_elm) {
   "use strict";
   _elm.Repos = _elm.Repos || {};
   if (_elm.Repos.values) return _elm.Repos.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var alertView = F2(function (address,msg) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("error alert")]),
      _U.list([$Html.text(msg)]));
   });
   var loadingView = function (address) {
      return A2($Html.div,
      _U.list([]),
   ...
   var headerView = F2(function (address,model) {
      ...
      _U.list([A2($Html.img,
              ...
              _U.list([]))
              ,A2($Html.h1,
              _U.list([$Html$Attributes.$class("headline")]),
              _U.list([$Html.text("Repos")]))
              ,A2($Html.form,
              _U.list([A2(onSubmit,address,model.userName)
                      ,$Html$Attributes.$class("search-form")]),
              _U.list([A2($Html.span,
                      _U.list([$Html$Attributes.$class("hint")]),
                      _U.list([$Html.text("github.com/")]))
                      ,A2($Html.input,
                      _U.list([$Html$Attributes.value(model.userName)
                              ,A2(onInput,address,NameChanged)
                              ,$Html$Attributes.$class("search-field")]),
                      _U.list([]))
                      ,A2($Html.button,
                      _U.list([$Html$Attributes.type$("submit")
                              ,$Html$Attributes.$class("submit-btn")]),
                      _U.list([$Html.text("Go")]))]))]));
   });
   var NoOp = {ctor: "NoOp"};
   var httpErrorToString = F2(function (name,err) {
      var _p1 = err;
      switch (_p1.ctor)
      {case "Timeout": return "Timeout";
         ...
         default: var _p2 = _p1._0;
           if (_p2 === 404) {
                 return A2($Basics._op["++"],name," found:(");
              } else {
                 return _p1._1;
              }}
   });
   var httpResultToAction = F2(function (name,result) {
      var _p3 = result;
      if (_p3.ctor === "Ok") {
            return FetchDone(_p3._0);
         } else {
            return Error(A2(httpErrorToString,name,_p3._0));
         }
   });
   ...
   var repoView = F3(function (address,model,repo) {
      var cloneValue = A2($Basics._op["++"],
      "git clone ",
      repo.sshUrl);
      var classNames = A2(isSelected,
      model,
      repo) ? "repo selected" : "repo";
      return A2($Html.li,
      _U.list([$Html$Attributes.$class(classNames)]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("repo-main")
                      ,A2($Html$Events.onClick,address,SelectRepo(repo))]),
              _U.list([A2($Html.img,
                      _U.list([$Html$Attributes.src(repo.avatarUrl)
              ...
              _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("clone")]),
              _U.list([A2($Html.label,
              _U.list([$Html$Attributes.$class("clone-label")]),
              _U.list([$Html.text("clone:")
                      ,A2($Html.input,
                      _U.list([$Html$Attributes.$class("clone-input")
                              ,$Html$Attributes.disabled(true)
                              ,$Html$Attributes.value(cloneValue)]),
                      _U.list([]))]))]))]))]));
   });
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([]),
      _U.list([A2($Html.a,
              _U.list([$Html$Attributes.href("https://github.com/turboMaCk/elm-github-repos")
                      ,$Html$Attributes.$class("fork-link")]),
              ...
              _U.list([A2(headerView,address,model)
                      ...
                      "Results for `",
                      A2($Basics._op["++"],model.resultsFor,"`:")))]))
   ...
   var repoDecoder = A7($Json$Decode.object6,
   Repo,
   ...
   A2($Json$Decode._op[":="],"ssh_url",$Json$Decode.string),
   A2($Json$Decode._op[":="],"stargazers_count",$Json$Decode.$int),
   A2($Json$Decode.at,
   _U.list(["owner","avatar_url"]),
   $Json$Decode.string));
   var reposDecoder = $Json$Decode.list(repoDecoder);
   var fetchData = function (name) {
      return $Task.toResult(A2($Http.get,
      reposDecoder,
      getUrl(name)));
   };
   var fetchDataAsEffects = function (name) {
      return $Effects.task(A2($Task.map,
      httpResultToAction(name),
      fetchData(name)));
   };
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return {ctor: "_Tuple2"
                           ,_0: model
                           ,_1: $Effects.none};
         case "FetchData": return {ctor: "_Tuple2"
                                  ,_0: _U.update(model,
   ...
   return _elm.Repos.values = {_op: _op
                              ...
                              ,getUrl: getUrl
                              ,fetchData: fetchData
                              ,httpErrorToString: httpErrorToString
